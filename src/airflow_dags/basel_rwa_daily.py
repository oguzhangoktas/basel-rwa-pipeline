...
def reconcile_results(**context):
    ...
    variance = abs(today_summary['total_exposure'][0] - source_totals['total_balance'][0]) / source_totals['total_balance'][0]
    
    if variance > 0.0001:
        raise ValueError(f"Reconciliation failed: {variance*100:.4f}% variance")
    
    # Compare with previous day (should not change > 10%)
    prev_day_summary = pd.read_sql(f"""
    ...
    if day_over_day_change > 0.1:
        ...
        send_slack_alert(f"⚠️ RWA changed {day_over_day_change*100:.2f}% from previous day")
...
# Define dependencies
[wait_for_loans, wait_for_collateral, wait_for_ratings, wait_for_market_data] >> validate_data
