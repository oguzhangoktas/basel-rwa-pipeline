...
rwa_detail = spark.read.format('delta') \
...
summary_asset_geo = summary_asset_geo \
    .withColumn('capital_ratio', 
                F.col('total_regulatory_capital') / F.col('total_exposure')) \
    .withColumn('rwa_density',
                F.col('total_rwa') / F.col('total_exposure'))
...
window_spec = Window.partitionBy('asset_class', 'geography', 'rating_bucket') \
    .orderBy('calculation_date')

trend_df = summary_asset_geo \
    .unionByName(historical_data, allowMissingColumns=True) \
    .withColumn('prev_total_rwa', F.lag('total_rwa', 1).over(window_spec)) \
    .withColumn('rwa_change_pct',
                ((F.col('total_rwa') - F.col('prev_total_rwa')) / F.col('prev_total_rwa') * 100)) \
    .filter(F.col('calculation_date') == execution_date) \
    .drop('prev_total_rwa')
...
total_capital_req = rwa_detail.agg(
    F.sum('risk_weighted_assets').alias('total_rwa'),
...
exposure_class_summary = rwa_detail.groupBy(
    'calculation_date',
    'asset_class'
).agg(
...
top_customers = rwa_detail.groupBy('customer_id').agg(
    F.sum('risk_weighted_assets').alias('customer_rwa'),
    F.sum('outstanding_balance_usd').alias('customer_exposure'),
    F.count('loan_id').alias('loan_count')
).orderBy(F.desc('customer_rwa')).limit(20) \
 .withColumn('calculation_date', F.lit(execution_date).cast('date'))

# Geographic Concentration
geo_concentration = rwa_detail.groupBy('geography').agg(
...
], ['calculation_date', 'total_rwa', 'total_capital', 'total_exposure', 
    'total_loans', 'total_customers', 'high_risk_loans', 'avg_risk_weight'])
...
trend_df.write \
    ...
    .save(f"{PROCESSED_PATH}/rwa_summary/")

# Regulatory reports
exposure_class_summary.write \
    .format('delta') \
    .mode('overwrite') \
    .option('replaceWhere', f"calculation_date = '{execution_date}'") \
...
kpi_summary.write \
    .format('delta') \
    .mode('append') \
    .save(f"{PROCESSED_PATH}/kpi_dashboard/")
...
trend_df.coalesce(10) \
    .write \
    .mode('overwrite') \
    .parquet(f"s3://bank-datalake/redshift-staging/rwa_summary/date={execution_date}/")

rwa_detail.select(
    'calculation_date', 'loan_id', 'customer_id', 'asset_class', 'geography',
    'outstanding_balance_usd', 'risk_weighted_assets', 'regulatory_capital',
    'risk_weight', 'rating_bucket', 'high_risk_flag'
).coalesce(50) \
 .write \
 .mode('overwrite') \
 .parquet(f"s3://bank-datalake/redshift-staging/rwa_detail/date={execution_date}/")
