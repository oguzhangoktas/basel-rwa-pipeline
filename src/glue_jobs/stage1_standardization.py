...
ratings_df = spark.read.csv(
    f"{RAW_PATH}/ratings/date={execution_date}/", header=True, inferSchema=True
)
...
loans_clean = (
    loans_df.withColumn(
        "outstanding_balance", F.col("outstanding_balance").cast("decimal(18,2)")
    )
...
# Add derived fields
loans_clean = (
    loans_clean.withColumn(
        "days_to_maturity", F.datediff(F.col("maturity_date"), F.lit(execution_date))
    )
    .withColumn(
        "loan_age_days", F.datediff(F.lit(execution_date), F.col("origination_date"))
    )
    .withColumn(
        "asset_class",
        F.when(F.col("product_type").isin(["MORTGAGE", "HOME_LOAN"]), "RETAIL_MORTGAGE")
        .when(
            F.col("product_type").isin(["PERSONAL_LOAN", "CREDIT_CARD"]), "RETAIL_OTHER"
        )
...
collateral_clean = (
    collateral_df.withColumn(
        "market_value", F.col("market_value").cast("decimal(18,2)")
    )
...
collateral_clean = collateral_clean.withColumn(
    "valuation_stale_flag",
    F.when(F.col("days_since_valuation") > 365, True).otherwise(False),
)

# Standardize counterparty
counterparty_clean = counterparty_df.withColumn(
    "customer_type", F.upper(F.trim(F.col("customer_type")))
).withColumn("total_assets", F.col("total_assets").cast("decimal(18,2)"))

# Standardize ratings - handle multiple agencies
ratings_clean = ratings_df.withColumn(
    "rating_agency", F.upper(F.trim(F.col("rating_agency")))
).withColumn("rating", F.upper(F.trim(F.col("rating"))))

# Deduplicate ratings (take most recent per customer per agency)
window_spec = Window.partitionBy("customer_id", "rating_agency").orderBy(
    F.col("rating_date").desc()
)
ratings_clean = (
    ratings_clean.withColumn("rank", F.row_number().over(window_spec))
    .filter(F.col("rank") == 1)
    .drop("rank")
)
...
rating_map_expr = F.create_map(
    [F.lit(x) for pair in rating_mapping.items() for x in pair]
)
ratings_clean = ratings_clean.withColumn(
    "rating_numeric", rating_map_expr[F.col("rating")]
)

# Convert FX rates to standard format
fx_rates_clean = fx_rates_df.withColumn(
    "currency_pair", F.upper(F.trim(F.col("currency_pair")))
...
loans_clean.repartition(200, "asset_class", "country_code").write.format("delta").mode(
    "overwrite"
).option("overwriteSchema", "true").partitionBy("asset_class").save(
    f"{STAGING_PATH}/loans_clean/"
)

collateral_clean.repartition(50, "collateral_type").write.format("delta").mode(
    "overwrite"
).partitionBy("collateral_type").save(f"{STAGING_PATH}/collateral_clean/")

counterparty_clean.repartition(20).write.format("delta").mode("overwrite").save(
    f"{STAGING_PATH}/counterparty_clean/"
)

ratings_clean.write.format("delta").mode("overwrite").save(
    f"{STAGING_PATH}/ratings_clean/"
)

fx_rates_clean.write.format("delta").mode("overwrite").save(
    f"{STAGING_PATH}/fx_rates_clean/"
)
