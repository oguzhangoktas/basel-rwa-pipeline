...
loans_usd = loans_df \
    .join(fx_pivot, loans_df.currency == fx_pivot.quote_ccy, 'left') \
    .withColumn('fx_rate', F.coalesce(F.col('fx_rate'), F.lit(1.0))) \
    .withColumn('outstanding_balance_usd', 
                F.col('outstanding_balance') * F.col('fx_rate')) \
    .drop('fx_rate', 'quote_ccy')

# Convert collateral values to USD
collateral_usd = collateral_df \
    .join(fx_pivot, collateral_df.currency == fx_pivot.quote_ccy, 'left') \
    .withColumn('fx_rate', F.coalesce(F.col('fx_rate'), F.lit(1.0))) \
    .withColumn('adjusted_collateral_value_usd',
...
loans_salted = loans_usd \
    .join(skewed_customers, 'customer_id', 'left') \
    .withColumn('is_skewed', F.when(F.col('loan_count').isNotNull(), True).otherwise(False)) \
    .withColumn('salt', 
                F.when(F.col('is_skewed'), 
                       (F.rand() * 10).cast('int'))
                 .otherwise(F.lit(0))) \
...
counterparty_salted = counterparty_df \
    .join(skewed_customers, 'customer_id', 'inner') \
    .withColumn('salt', F.explode(F.array([F.lit(i) for i in range(10)]))) \
    .withColumn('customer_id_salted',
                F.concat(F.col('customer_id'), F.lit('_'), F.col('salt'))) \
    .unionByName(
        counterparty_df
            .join(skewed_customers, 'customer_id', 'left_anti')
            .withColumn('salt', F.lit(0))
            .withColumn('customer_id_salted', F.col('customer_id'))
    )

# Join with counterparty (salted to handle skew)
enriched_df = loans_salted \
    .join(counterparty_salted, 'customer_id_salted', 'left') \
    .drop('customer_id_salted', 'salt', 'is_skewed', 'loan_count')

# Join with collateral (broadcast join - smaller table)
enriched_df = enriched_df \
    .join(collateral_usd, 'collateral_id', 'left')

# Join with external ratings (broadcast)
ratings_broadcast = F.broadcast(ratings_df)
enriched_df = enriched_df \
    .join(ratings_broadcast, 'customer_id', 'left')
...
enriched_df = enriched_df \
    .withColumn('exposure_type',
                F.when(F.col('product_type').isin(['COMMITTED_LINE']), 'COMMITTED_LINE')
                 .otherwise('ON_BALANCE'))

enriched_df = enriched_df \
    .join(ccf_map, 'exposure_type', 'left') \
    .withColumn('ccf', F.coalesce(F.col('ccf'), F.lit(1.0))) \
    .withColumn('exposure_at_default',
                F.col('outstanding_balance_usd') * F.col('ccf'))

# 5.3: Calculate Credit Risk Mitigation (CRM) from Collateral
enriched_df = enriched_df \
    .withColumn('collateral_coverage_ratio',
                F.when(F.col('adjusted_collateral_value_usd').isNotNull(),
                       F.least(
...
enriched_df = enriched_df \
    .withColumn('ead_after_crm',
                F.col('exposure_at_default') * (1 - F.col('collateral_coverage_ratio')))

# 5.4: Join Risk Weights
enriched_df = enriched_df \
    .join(risk_weight_map, ['asset_class', 'rating_bucket'], 'left') \
    .withColumn('risk_weight',
                F.coalesce(F.col('risk_weight'), F.lit(1.0)))  # Default 100% if not found

# 5.5: Calculate Risk-Weighted Assets (RWA)
enriched_df = enriched_df \
    .withColumn('risk_weighted_assets',
                F.col('ead_after_crm') * F.col('risk_weight'))

# 5.6: Calculate Regulatory Capital Requirement (8% of RWA per Basel III)
enriched_df = enriched_df \
    .withColumn('regulatory_capital',
                F.col('risk_weighted_assets') * F.lit(0.08))
...
pd_mapping = {
    ...
    'BB': 0.005, 'B': 0.02, 'CCC': 0.10, 'DEFAULT': 1.0
}
pd_map_expr = F.create_map([F.lit(x) for pair in pd_mapping.items() for x in pair])

enriched_df = enriched_df \
    .withColumn('probability_of_default',
                pd_map_expr[F.col('rating_bucket')])

# 5.8: Join LGD
enriched_df = enriched_df \
    .join(lgd_map, 'collateral_type', 'left') \
    .withColumn('loss_given_default',
                F.coalesce(F.col('lgd'), F.lit(0.45)))  # Default LGD 45%
...
enriched_df = enriched_df \
    .withColumn('risk_weight_floored',
                F.when(F.col('asset_class') == 'RETAIL_MORTGAGE',
                       F.greatest(F.col('risk_weight'), F.lit(0.10)))  # 10% floor for mortgages
                 .otherwise(F.col('risk_weight'))) \
    .withColumn('risk_weighted_assets',
                F.col('ead_after_crm') * F.col('risk_weight_floored'))

# 6.2: Flag high-risk loans for review
enriched_df = enriched_df \
    .withColumn('high_risk_flag',
                F.when((F.col('risk_weight') > 1.0) | 
                       (F.col('probability_of_default') > 0.05) |
                       (F.col('days_to_maturity') < 30), True)
                 .otherwise(False))
...
enriched_df = enriched_df \
    ...
    .withColumn('data_quality_score',
                F.when(F.col('rating').isNotNull(), F.lit(1.0))  # Has external rating
                 ...
                 .otherwise(F.lit(0.5)))  # Default rating used
...
validation_df = enriched_df.agg(
    ...
    F.sum(F.when(F.col('high_risk_flag') == True, 1).otherwise(0)).alias('high_risk_count')
)
...
assert validation_metrics['total_rwa'] <= validation_metrics['total_exposure'] * 1.5, \
...
output_df \
    .repartition(100, 'asset_class', 'geography') \
    .write \
    .format('delta') \
    .mode('overwrite') \
    .option('replaceWhere', f"calculation_date = '{execution_date}'") \
...
deltaTable.optimize() \
    .where(f"calculation_date = '{execution_date}'") \
...
stats_df \
    ...
    .mode('overwrite') \
    .option('replaceWhere', f"calculation_date = '{execution_date}'") \
    .save(f"{PROCESSED_PATH}/rwa_statistics/")
