...
class TestRWACalculations:
    ...
    def test_rwa_aaa_corporate(self, spark):
        ...
        result_df = df.withColumn(
            "ead",
            F.col("outstanding_balance") * F.col("ccf")
        ).withColumn(
            "ead_after_crm",
            F.col("ead") * (1 - F.col("collateral_coverage"))
        ).withColumn(
            "risk_weight",
            F.lit(0.20)  # Basel III: AAA corporate = 20%
        ).withColumn(
            "rwa",
    ...
    def test_rwa_high_risk_unsecured(self, spark):
        ...
        result_df = df.withColumn(
            ...
            "risk_weight",
            F.lit(1.50)  # B-rated corporate
        ).withColumn(
            "rwa",
            F.col("ead") * F.col("risk_weight")
        )
    ...
    def test_expected_loss_calculation(self, spark):
        ...
        result_df = df.withColumn(
            "expected_loss",
            F.col("ead") * F.col("probability_of_default") * F.col("loss_given_default")
        )
    ...
    def test_collateral_coverage_cap(self, spark):
        ...
        data = [(
            ...
            "C008",
            100_000.0,
            1.5  # 150% coverage - should be capped at 100%
        )]
        ...
        result_df = df.withColumn(
            "collateral_coverage",
            F.least(F.col("collateral_coverage_raw"), F.lit(1.0))
        ).withColumn(
            "ead_after_crm",
            F.col("ead") * (1 - F.col("collateral_coverage"))
        )
...
class TestPerformanceOptimizations:
    ...
    def test_salting_for_skewed_keys(self, spark):
        ...
        # When: Apply salting
        salted_df = df.join(
            F.broadcast(skewed_customers.select("customer_id")),
            "customer_id",
            "left"
        ).withColumn(
            "salt",
